			...
			<c:if test="${not empty list}">
				<c:forEach var="notice" items="${list }">
					<tr>
						...
						<c:if test="${notice.n_del != 'y' }">
							<td><a
								href="${path }/adminBoard/adminNoticeView.do?nno=${notice.nno}&pageNum=${pb.currentPage}">${notice.n_title }</a></td>
							<td>${notice.n_cnt }</td>
							<td>${notice.n_date }</td>
							...
							<td><button class="btn btn-sm btn-dark" onclick="del(${notice.nno})">삭제</button></td>
						</c:if>
					</tr>
				</c:forEach>
			</c:if>
